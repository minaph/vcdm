...
def make_report(device_tensor_stat: dict, verbose: bool = False, target_device: Optional[torch.device] = None) -> None:
    ...
    result = []
    result.append(template_format.format('Element type', 'Size', 'Used MEM'))
    for device, tensor_stats in device_tensor_stat.items():
        ...
        if target_device is not None and device != target_device:
            continuresult += ('-' * LEN)
        result.append('Storage on {}'.format(device))
        total_mem = 0
        total_numel = 0
        for stat in tensor_stats:
            ...
            if not show_reuse:
                name = name.split('(')[0]
            result.append(template_format.format(
                str(name),
                str(size),
                readable_size(mem),
            ))
        ...
        result.append('-'*LEN)
        result.append('Total Tensors: {} \tUsed Memory: {}'.format(
        ...
        if device != torch.device('cpu'):
            ...
            result.append('The allocated memory on {}: {}'.format(
                device, readable_size(memory_allocated),
            ))
            if memory_allocated != total_mem:
                result.append('Memory differs due to the matrix alignment or'
